...
int TempPin = A5; // select the input pin A0 for the Temperature input
int TempValue = 0; // variable to store the value coming from the Temperature Sensor
int Warning = A4; // select the input pin A4 for the LDR input
void setup() {
pinMode(ldrPin, INPUT); // Set up the pin as an input pin
pinMode(TempPin, INPUT); // Set up the pin as an input pin
pinMode(Warning, OUTPUT); // Set up the pin as an input pin
...
int Temp = (((TempValue * 4.965 * 100.0)/1024)-273.15);
int TempK = (((TempValue * 4.965 * 100.0)/1024));
Serial.print("Temperature (°C):"); // no 'ln' as I don't want a new line yet
Serial.println(Temp); // print out the LDR value
Serial.print("Temperature (K):"); // no 'ln' as I don't want a new line yet
Serial.println(TempK); // print out the LDR value

if (ldrValue <= 10 && TempValue == 1023){
digitalWrite(Warning, HIGH);
Serial.println("LDR Failure Detected"); // print out the LDR value
Serial.println("Temp Sensor Failure Detected"); // print out the LDR value
}
